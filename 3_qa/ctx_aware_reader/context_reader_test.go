package ctx_aware_reader_test

import (
	"context"
	"learn-go-with-tests/3_qa/ctx_aware_reader"
	"strings"
	"testing"
)
...
func TestContextAwareReader(t *testing.T) {
	t.Run("behaves like a normal reader", func(t *testing.T) {
		rdr := ctx_aware_reader.NewCancellableReader(context.Background(), strings.NewReader("123456"))
		got := make([]byte, 3)
	...
	t.Run("stops reading when cancelled", func(t *testing.T) {
		...
		rdr := ctx_aware_reader.NewCancellableReader(ctx, strings.NewReader("123456"))
		got := make([]byte, 3)
		...
		n, err := rdr.Read(got)
		if err == nil {
			t.Errorf("expected an error after cancellation but didn't get one")
		}
