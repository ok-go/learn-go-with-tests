...
func TestGetPlayers(t *testing.T) {
	...
	t.Run("returns Pepper's score", func(t *testing.T) {
		...
		poker.AssertStringEquals(t, response.Body.String(), "20")
	})
	t.Run("returns Floyd's score", func(t *testing.T) {
		...
		poker.AssertStatus(t, response, http.StatusOK)
		poker.AssertStringEquals(t, response.Body.String(), "10")
	})
...
func assertWebSocketGotMessage(t *testing.T, ws *websocket.Conn, want string) {
	...
	if string(msg) != want {
		t.Errorf("read message from WebSocket: got %q, want %q", string(msg), want)
	}
}
